...
export function EventCard({ event, onEdit, onDelete, onStatusChange, index }: EventCardProps) {
  ...
  const handleStatusClick = () => {
    ...
    // If next status is "paid", show confirmation dialog
    if (next === 'paid') {
        ...
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              האם לסמן את האירוע של <strong>{event.clientName}</strong> כשולם?
              <br />
              <span className="text-lg font-bold text-foreground">₪{event.price.toLocaleString()}</span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-row-reverse gap-2">
            <AlertDialogAction onClick={handleConfirmPaid} className="bg-success hover:bg-success/90">
              ✓ כן, שולם
            </AlertDialogAction>
